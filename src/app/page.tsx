...
export default async function Home() {
  ...
  type CategoryWithProducts = (typeof categoriesData)[0] & {
    products: Product[];
  };

  const categories: CategoryWithProducts[] = categoriesData
    .map((category) => ({
      ...category,
  ...
  return (
    <div className='flex flex-col min-h-screen pb-20 md:pb-0 bg-white selection:bg-blue-500/20'>
      ...
      <section className='relative h-[600px] flex items-center bg-slate-900 border-b border-white/10'>
        ...
        publishedProducts[0].images &&
        publishedProducts[0].images.length > 0 ? (
          <>
        ...
        <div className='container mx-auto px-4'>
          <div className='grid grid-cols-2 md:grid-cols-4 divide-x divide-slate-100 border-x border-slate-100'>
            {stats.map((stat, index) => (
              <div
                key={index}
                className='p-6 md:p-8 flex flex-col items-center justify-center text-center group hover:bg-slate-50 transition-colors'>
          ...
          <div className='grid md:grid-cols-2 gap-12 items-center'>
            <div>
              ...
              <div className='space-y-4 text-slate-600 leading-relaxed'>
                <p>
                  {config.companyName}{' '}
                  是一家集研发、生产、销售为一体的现代化水处理企业。我们在河南拥有两大生产基地，年产能超过10万吨。
                </p>
                <p>
        ...
        <div className='container mx-auto px-4'>
          ...
          <div className='grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8'>
            {categories.map((category) => (
              <div
                key={category.id}
                className='group bg-white rounded-sm border border-slate-200 hover:border-blue-500/30 hover:shadow-lg transition-all duration-300 flex flex-col h-full'>
                <div className='p-6 border-b border-slate-100 bg-slate-50/50 group-hover:bg-blue-50/10 transition-colors'>
                  <div className='flex justify-between items-start mb-4'>
                    <div className='w-12 h-12 bg-white border border-slate-200 rounded-sm flex items-center justify-center shadow-sm group-hover:bg-blue-600 group-hover:border-blue-600 transition-colors'>
                      {/* 简单的分类图标逻辑 */}
                      {category.id === '1' ? (
                        <Factory className='w-6 h-6 text-slate-600 group-hover:text-white' />
                      ) : category.id === '2' ? (
                        <ShieldCheck className='w-6 h-6 text-slate-600 group-hover:text-white' />
                      ) : (
                    ...
                    <Badge
                      variant='secondary'
                      className='bg-slate-100 text-slate-600 hover:bg-slate-200 border-0 rounded-sm'>
                ...
                <div className='p-6 flex-1 flex flex-col gap-3'>
                  {category.products.slice(0, 3).map((product) => (
                    <Link
                      ...
                      <span className='text-sm font-medium text-slate-700 group-hover/item:text-blue-600 transition-colors line-clamp-1'>
                        {product.name}
                      </span>
                ...
                <div className='p-4 pt-0 mt-auto'>
                  <Button
                    variant='ghost'
                    asChild
                    className='w-full justify-between text-slate-500 hover:text-blue-700 hover:bg-transparent px-2'>
      ...
      <section className='py-16 md:py-24 bg-slate-900 text-white'>
        <div className='container mx-auto px-4'>
          <div className='flex flex-col md:flex-row justify-between items-end mb-12'>
            ...
            <Button
              variant='outline'
              className='hidden md:flex border-slate-700 text-slate-300 hover:bg-white hover:text-slate-900 rounded-sm'>
          ...
          <div className='grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6'>
            {[
              ...
              <div
                key={idx}
                className='bg-slate-800/50 p-6 rounded-sm border border-slate-700 hover:border-blue-500 hover:bg-slate-800 transition-all group'>
                <div className='flex justify-between items-start mb-4'>
                  <Badge className='bg-blue-900/50 text-blue-300 border-blue-800/50 rounded-sm'>
                    {item.tag}
                  </Badge>
          ...
          <div className='mt-8 md:hidden'>
            <Button
              variant='outline'
              className='w-full border-slate-700 text-slate-300 hover:bg-white hover:text-slate-900 rounded-sm'>
              <Link href='/contact'>索取更多案例</Link>
            </Button>
          </div>
        </div>
        ...
        <div className='container mx-auto px-4'>
          ...
          <div className='grid grid-cols-2 md:grid-cols-4 lg:grid-cols-6 gap-6 opacity-70'>
            ...
            {[
              ...
              <div
                key={idx}
                className='h-20 border border-slate-200 rounded-sm flex items-center justify-center bg-slate-50 font-semibold text-slate-500 text-sm hover:bg-white hover:shadow-md transition-all cursor-default'>
                {item}
              </div>
            ))}
          </div>
      ...
      <section className='bg-slate-50 py-20'>
        <div className='container mx-auto px-4 text-center'>
          ...
          <div className='flex flex-col sm:flex-row gap-4 justify-center'>
            <Button
              size='lg'
              className='h-14 px-8 text-base bg-blue-600 hover:bg-blue-700 rounded-sm text-white hover:-translate-y-1 transition-all shadow-lg shadow-blue-600/20'>
            ...
            <Button
              size='lg'
              variant='outline'
              className='h-14 px-8 text-base bg-white border-slate-300 text-slate-700 hover:bg-slate-50 rounded-sm hover:-translate-y-1 transition-all'>
              <Link
