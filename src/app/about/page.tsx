...
export default async function AboutPage() {
  ...
  return (
    <div className="container mx-auto py-6 sm:py-8 md:py-12 px-4 pb-24 md:pb-12">
      <h1 className="text-2xl sm:text-3xl font-bold mb-6 sm:mb-8">关于我们</h1>
      <div className="prose prose-slate prose-sm sm:prose-base max-w-none text-slate-700 leading-relaxed space-y-4 sm:space-y-6">
        {config.aboutUs ? (
          <ReactMarkdown>{config.aboutUs}</ReactMarkdown>
        ) : (
          <>
            <p>
              <strong>{config.companyName}</strong>集科研、生产、经营、服务于一体，是从事水处理剂的开发、生产、经营及从事水处理工程的设计、研发、运营及各类废水，噪音治理的专业企业。
            </p>
            <p>
            ...
            <p className="font-bold text-base sm:text-lg text-primary mt-6 sm:mt-8">
              敬业 团队 创新 诚信服务
            </p>
          </>
        )}
